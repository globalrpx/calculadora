...
| `/admin/simulacoes-finais/[id]` | Admin | Parcial | `final_simulations`, `simulation_tax_lines`, `simulation_documents` | Detalhe com dados principais, produtos, despesas, parametrização fiscal, recálculo fiscal V1, resumo salvo, linhas fiscais por produto, geração de relatório interno e histórico de documentos gerados. |
...
| `/admin/simulacoes-finais/[id]/preview-cliente` | Admin | Parcial | `final_simulations`, `final_simulation_items`, `simulation_tax_lines`, `simulation_expense_lines`, `calculation_snapshot`, `public_snapshot`, `internal_snapshot` | Preview HTML do PDF cliente para validação visual, geração dos snapshots, PDF cliente salvo e relatório interno simples a partir dos snapshots. |
...
| `/admin/simulacoes-finais/[id]/documentos/[documentId]/pdf` | Admin | Parcial | `simulation_documents`, Supabase Storage privado | Serve PDFs salvos no bucket `app-uploads` para `client_pdf` e `internal_detailed_report`; suporta visualização inline e download via `?download=1`. |

...
| `/admin/simulacoes-finais/[id]` | Admin | Parcial | `final_simulations`, `simulation_tax_lines`, `simulation_documents` | Detalhe com dados principais, produtos, despesas, parametrização fiscal, recálculo fiscal V1, resumo salvo, linhas fiscais por produto e histórico de PDFs cliente gerados. |
...
| `/admin/simulacoes-finais/[id]/documentos/[documentId]/pdf` | Admin | Parcial | `simulation_documents`, Supabase Storage privado | Serve PDF cliente salvo no bucket `app-uploads`; suporta visualização inline e download via `?download=1`. |

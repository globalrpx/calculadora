...
import type { ClientQuoteRecord, ClientSimulationRecord } from "@/lib/client/types";
...
export async function getClientSimulations(): Promise<ClientSimulationRecord[]> {
  ...
  const { data } = await supabase
    .from("simulations")
    .select("id, title, status, quote_file_url, storage_path, created_at, quote:quotes(id, product_name, hs_code)")
    .eq("client_id", appUser.client_id)
    .order("created_at", { ascending: false });
  ...
  return (data ?? []).map((row) => {
    ...
    return {
      ...
      title: row.title,
      status: row.status,
      quoteFileUrl: row.quote_file_url ?? row.storage_path ?? null,
      createdAt: row.created_at,
      quote: quote
        ? {
            id: quote.id,
